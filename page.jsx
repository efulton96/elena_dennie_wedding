...
// Lightweight local UI primitives to avoid external dependencies in this demo.
...
// Minimal icon fallbacks (emoji or simple SVG) to avoid lucide-react dependency.
...
/**
 * WEDDING WEBSITE – SINGLE FILE STARTER (React + Tailwind + shadcn/ui + framer-motion)
 *
 * ✨ How to use
 * 1) Edit the CONFIG object below to change names, colors, sections, copy, links, and images.
 * 2) This file renders a complete single‑page site you can deploy with Next.js, Vite, Netlify, or Vercel.
 * 3) The RSVP form is client‑side only by default. See "hook up RSVP" notes to connect to Netlify Forms, Airtable, Google Sheets, or Email.
 * 4) Add/remove sections by toggling CONFIG.sections.* booleans or reordering the <Section> blocks.
 *
 * 🛠️ Next steps I can help wire up on request: password protection, custom domain, analytics, SEO, ADA checks, print‑friendly schedule, photo gallery lightbox, and a headless CMS (Notion, Sanity, Contentful) so you can edit without touching code.
 */

const CONFIG = {
  couple: {
    primaryNames: "Elena & Dennie", // shown in hero
    fullNames: "Elena Fulton & Dennie",
  },
  date: {
    iso: "2026-10-16",
    pretty: "October 16, 2026",
    dayTime: "Friday 5:30 PM, PST",
  },
  venue: {
    name: "Lionsgate Event Center – The Gatehouse",
    address: "1055 US-287, Lafayette, CO 80026",
    cityState: "Lafayette, Colorado",
    mapsUrl: "https://maps.google.com/?q=Lionsgate+Event+Center+Gatehouse",
  },
  accommodations: {
    note: "We reserved a block at the Residence Inn Boulder Broomfield/Interlocken.",
    hotel: {
      name: "Residence Inn Boulder Broomfield/Interlocken",
      address: "455 Zang St, Broomfield, CO 80021",
      mapsUrl: "https://maps.google.com/?q=Residence+Inn+Boulder+Broomfield/Interlocken",
      shuttleNote:
        "Complimentary shuttle will run at set intervals between the hotel and venue on the wedding day.",
    },
  },
  travel: {
    airport: {
      ...
      blurb:
        "From DEN, take I‑70 W → I‑270 W → US‑36 W toward Boulder, then exit to US‑287 S toward Lafayette. Typical drive ~40–55 min depending on traffic.",
      toHotel:
        "From DEN to the Residence Inn: I‑70 W → I‑270 W → US‑36 W to Interlocken Loop. Follow signs to Zang St.",
      toVenue:
        "From DEN to Lionsgate: I‑70 W → I‑270 W → US‑36 W → exit for US‑287 S toward Lafayette. Venue entrance is on US‑287.",
      parkingNote:
        "Venue has on‑site parking. Rideshare pickup is straightforward at the Gatehouse entrance.",
    },
  },
  registry: [
    { label: "Zola", url: "https://www.zola.com/registry/your‑registry" },
    { label: "Target", url: "https://www.target.com/gift‑registry/" },
  ],
  colors: {
    // Ivory base + your palette (deep teal, forest, rust, wine, plum)
    bg: "#F8F5EF", // soft ivory
    accent: "#F3EEE6", // lighter ivory gradient tail
    primary: "#1F4C49", // deep teal / phthalo-leaning green
    secondary: "#123417", // forest green
    highlight: "#7E3A22", // rust
    text: "#1A1E23", // ink
    wine: "#461326", // deep wine
    plum: "#3E0B4D", // plum
  },
  hero: {
    tagline: "Friday, October 16th 2026",
    backgroundUrl: "/img/DSCF8151.jpg", // replace with your photo
    height: { mobile: "80vh", desktop: "10000vh" },
    focus: "center bottom",
    scale: 1,
    content: {
      align: "flex-start",
      justify: "flex-end",
      paddingTop: "var(--sticky-h, 72px)",
      paddingBottom: "3rem",
      paddingLeft: "0",
      paddingRight: "0",
      textAlign: "left",
  ...
  faq: [
    {
      ...
      a: "Dressy cocktail with jewel tone accents encouraged! The ceremony is outdoors, but there is space in the reception area for jackets.",
    },
    { q: "Are kids welcome?", a: "We love your little ones, but this will be an adults‑only celebration." },
    { q: "Is the venue accessible?", a: "Yes. The Gatehouse has accessible parking and entry." },
  ],
  schedule: [
  ...
  rsvp: {
    mode: "internal", // "external" | "embed"
    externalUrl: "",  // if mode === "external" (e.g., your Aisle Planner RSVP link)
    embed: { src: "", height: 900 }, // if mode === "embed" and allowed by provider
  },
  updates: {
    mode: "netlify", // "netlify" | "mailchimp" | "sheet"
    mailchimp: { actionUrl: "", fieldEmail: "EMAIL", fieldName: "FNAME" },
    sheet: { endpoint: "/api/updates" }
  },
  sections: {
    schedule: true,
    updates: true,
...
// ---------- UI PRIMITIVES ----------
const Section = ({ id, title, children, show = true }) => (show ? (
  <section
    id={id}
  ...
  >
    ...
    <div className="max-w-5xl mx-auto px-4 h-full flex flex-col justify-center section-inner">
      <div className="flex justify-center mb-6">
        <div className="section-heading nouveau-script text-lg md:text-xl" style={{ color: CONFIG.colors.text }}>
          {title}
        </div>
      </div>
...
const Anchor = ({ href, children }) => (
  <a className="underline underline-offset-2 hover:no-underline" href={href} target="_blank" rel="noreferrer">
    {children}
  </a>
);
...
// Fallback images used if remote URLs fail (e.g., Google Photos share links)
...
// --- Art Nouveau helpers (light Mucha-inspired flourishes) ---
...
// Delicate Mucha-inspired side vines
...
// Subtle hero flourish under the subtitle
...
export default function WeddingWebsiteStarter() {
  ...
  const heroHeightMobile = heroHeights.mobile || '80vh';
  const heroHeightDesktop = heroHeights.desktop || '100vh';
  ...
  const submitUpdatesNetlify = async (ev) => {
    ...
    if (!emailOk) {
      setUpdatesErrors({ email: "Enter a valid email." });
      return;
    }
    try {
      await fetch("/", {
        ...
        body: encode({ "form-name": "Updates", name: updatesForm.name, email: updatesForm.email }),
      });
  ...
  return (
    <>
      ...
      <link href="https://fonts.googleapis.com/css2?family=Italiana&family=Josefin+Sans:wght@400;500;600;700&family=Lora:wght@400;500;600&display=swap" rel="stylesheet" />
      <div className="page-ornament left" />
      <div className="page-ornament right" />
      <div className="min-h-screen font-serif" style={{ background: `linear-gradient(180deg, ${palette.bg} 0%, ${palette.accent} 100%)`, color: palette.text }}>
      {/* NAV */}
      <div className="sticky top-0 z-40 backdrop-blur supports-[backdrop-filter]:bg-white/70 border-b border-black/10">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between" style={{ fontFamily: "'Italiana','Lora',serif", fontSize: '1.05rem', fontWeight: 600 }}>
          <a href="#" onClick={(e)=>{e.preventDefault(); go('home');}} className="tracking-wide" style={{ fontWeight: 'inherit' }}>Home</a>
          <nav className="hidden md:flex items-center gap-6 nav-links" style={{ fontSize: 'inherit', fontWeight: 'inherit' }}>
            <a href="#" onClick={(e)=>{e.preventDefault(); go('details');}} className="hover:opacity-80">Details</a>
            {CONFIG.sections.schedule && <a href="#" onClick={(e)=>{e.preventDefault(); go('schedule');}} className="hover:opacity-80">Schedule</a>}
            {CONFIG.sections.travel && <a href="#" onClick={(e)=>{e.preventDefault(); go('travel');}} className="hover:opacity-80">Travel</a>}
            {CONFIG.sections.accommodations && <a href="#" onClick={(e)=>{e.preventDefault(); go('accommodations');}} className="hover:opacity-80">Stay</a>}
            {CONFIG.sections.registry && <a href="#" onClick={(e)=>{e.preventDefault(); go('registry');}} className="hover:opacity-80">Registry</a>}
            {CONFIG.sections.gallery && <a href="#" onClick={(e)=>{e.preventDefault(); go('gallery');}} className="hover:opacity-80">Gallery</a>}
            {CONFIG.sections.faq && <a href="#" onClick={(e)=>{e.preventDefault(); go('faq');}} className="hover:opacity-80">FAQ</a>}
            {CONFIG.sections.updates && <a href="#" onClick={(e)=>{e.preventDefault(); go('updates');}} className="hover:opacity-80">Updates</a>}
            
          </nav>
          <Button className="hidden md:inline-flex btn-primary rsvp-btn" style={{ backgroundColor: palette.primary }} onClick={()=>go('rsvp')}>RSVP</Button>
        </div>
      </div>

      {/* HERO */}
      {page==='home' && (<header id="top" className="relative" style={heroStyleVars}>
        <div className="absolute inset-0">
          <img src={CONFIG.hero.backgroundUrl} alt="Hero" onError={(e) => (e.currentTarget.src = FALLBACKS.hero)} className="hero-img w-full object-cover opacity-70" />
        ...
        <div className="relative max-w-5xl mx-auto px-4 hero-inner flex flex-col" style={heroContentStyle}>
          <h1 className="text-4xl md:text-6xl font-normal tracking-wide drop-shadow-md text-white" style={{ fontFamily: "'Italiana','Lora',serif", animation: 'fadeIn 600ms ease both' }}>
            {CONFIG.couple.primaryNames}
          </h1>
          <p className="mt-3 text-lg md:text-xl max-w-2xl text-white/90" style={{ animation: 'fadeIn 700ms ease both' }}>
            {CONFIG.hero.tagline}
          </p>
      ...
      <Section show={page==='details'} id="details" title="The Details">
        ...
        <div className="detail-grid">
          <div className="detail-card">
            ...
            <p style={{ fontSize: '1rem', lineHeight: '1.6' }}>Ceremony will be held on the lawn of the Gatehouse at sunset.</p>
          ...
          <div className="detail-grid">
            {CONFIG.registry.map((r, i) => (
              <a key={i} href={r.url} target="_blank" rel="noreferrer" className="detail-card" style={{ textDecoration: 'none', color: 'inherit' }}>
                <header style={{ display: 'flex', alignItems: 'center', justify-content: 'center', gap: '0.5rem' }}>
                  <span>{r.label}</span>
      ...
      {CONFIG.sections.updates && (
        <Section show={page==='updates'} id="updates" title="Get Wedding Updates">
          <div className="detail-card" style={{ maxWidth: '640px', margin: '0 auto', textAlign: 'left', background: 'rgba(31,76,73,0.9)', color: '#fff8f0' }}>
            ...
            {CONFIG.updates.mode === "mailchimp" ? (
            ...
            ) : CONFIG.updates.mode === "netlify" ? (
              updatesSubmitted ? (
                <div className="text-center space-y-3 py-10">
                  <h3 className="text-xl font-semibold">You're on the list!</h3>
                  <p className="opacity-80">We’ll email flight/hotel reminders and planning updates.</p>
                </div>
              ) : (
                <form name="Updates" method="POST" data-netlify="true" netlify-honeypot="bot-field" className="grid md:grid-cols-2 gap-4" onSubmit={submitUpdatesNetlify}>
                  ...
                  <div className="space-y-2">
                    ...
                    {updatesErrors.email && <p className="text-red-300 text-xs">{updatesErrors.email}</p>}
                  </div>
                  <div className="md:col-span-2 flex justify-end">
                    <Button type="submit" size="lg" style={{ backgroundColor: palette.primary }}>Subscribe</Button>
                  </div>
              ...
              updatesSubmitted ? (
                ...
                <form onSubmit={async (e) => { e.preventDefault(); try { await fetch(CONFIG.updates.sheet.endpoint, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(updatesForm) }); setUpdatesSubmitted(true); } catch (err) { setUpdatesErrors({ email: "Could not subscribe. Try again." }); } }} className="grid md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="updates-name2">First name</Label>
                    <Input id="updates-name2" value={updatesForm.name} onChange={(e) => setUpdatesForm({ ...updatesForm, name: e.target.value })} />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="updates-email2">Email</Label>
                    <Input id="updates-email2" type="email" value={updatesForm.email} onChange={(e) => setUpdatesForm({ ...updatesForm, email: e.target.value })} />
                    {updatesErrors.email && <p className="text-red-300 text-xs">{updatesErrors.email}</p>}
                  </div>
                  <div className="md:col-span-2 flex justify-end">
                    <Button type="submit" size="lg" style={{ backgroundColor: palette.primary }}>Subscribe</Button>
      ...
      {CONFIG.sections.rsvp && (
        <Section show={page==='rsvp'} id="rsvp" title="RSVP">
          <Card className="bg-white/95 border-black/10 rounded-3xl shadow-sm ring-1 ring-black/5">
            <CardContent className="pt-6">
              {CONFIG.rsvp.mode === "external" ? (
                <div className="text-center space-y-4 py-10">
                  <p className="opacity-90">Please submit your RSVP via our Aisle Planner form.</p>
                  <a href={CONFIG.rsvp.externalUrl} target="_blank" rel="noreferrer">
                    <Button size="lg" style={{ backgroundColor: palette.primary }}>Go to RSVP</Button>
                  </a>
                </div>
              ) : CONFIG.rsvp.mode === "embed" ? (
                <div className="py-2">
                  <iframe src={CONFIG.rsvp.embed.src} title="RSVP" className="w-full rounded-xl border" style={{ height: CONFIG.rsvp.embed.height }} />
                </div>
              ) : submitted ? (
                <div className="text-center space-y-3 py-10">
                  <h3 className="text-xl font-semibold">Thanks for your RSVP!</h3>
                  <p className="opacity-80 max-w-prose mx-auto">We’ve recorded your response for {CONFIG.couple.fullNames}. If you need to make changes, just submit the form again.</p>
                </div>
              ) : (
                <form onSubmit={submitForm} className="grid md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="name">Full Name</Label>
                    <Input id="name" value={rsvp.name} onChange={(e) => setRsvp({ ...rsvp, name: e.target.value })} placeholder="Jane Doe" />
                    {errors.name && <p className="text-red-300 text-xs">{errors.name}</p>}
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="email">Email</Label>
                    <Input id="email" type="email" value={rsvp.email} onChange={(e) => setRsvp({ ...rsvp, email: e.target.value })} placeholder="jane@example.com" />
                    {errors.email && <p className="text-red-300 text-xs">{errors.email}</p>}
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="attending">Will you attend?</Label>
                    <select id="attending" className="w-full bg-transparent border rounded-md px-3 py-2" value={rsvp.attending} onChange={(e) => setRsvp({ ...rsvp, attending: e.target.value })}>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="guests" type="number" min={1} value={rsvp.guests} onChange={(e) => setRsvp({ ...rsvp, guests: Number(e.target.value) })} />
                    {errors.guests && <p className="text-red-300 text-xs">{errors.guests}</p>}
                  </div>
                  <div className="md:col-span-2 space-y-2">
                    <Label htmlFor="message">Notes (allergies, songs, etc.)</Label>
                    <Textarea id="message" rows={4} value={rsvp.message} onChange={(e) => setRsvp({ ...rsvp, message: e.target.value })} placeholder="Add any details we should know" />
                  </div>
                  <div className="md:col-span-2 flex justify-end">
                    <Button type="submit" size="lg" style={{ backgroundColor: palette.primary }}>Submit RSVP</Button>
                  </div>
                </form>
              )}
            </CardContent>
